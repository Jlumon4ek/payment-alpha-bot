    ...
    async def update(self, telegram_id: int, **kwargs):
        async with get_session() as session:
            try:
                ...
                if not user:
                    logger.warning(f"User with telegram_id {telegram_id} not found in update_user.")
                    return
                ...
                try:
                    session.add(user)
                    await session.commit()
                    logger.info(f"User with telegram_id {telegram_id} successfully updated with wallets.")
